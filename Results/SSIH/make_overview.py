...
def fetch_data(mode):
    ...
    for domain in [d for d in os.listdir('.') if not os.path.isfile(d) and mode_condition[mode](d)]:
        ...
        try:
            ...
            print(f"No graph_data file found for domain {domain}")
            continue

        for instance_name in (f"./{domain}/Stats/pfile{i:0>2}--pfile{j:0>2}.csv" for (i, j) in
                              it.product(range(min_n, max_n + 1),
                                         range(min_n, max_n + 1)) if i >= j):  # instance_id_generator(min_n, max_n)):
            instance_stats = None
            try:
            ...
            if instance_stats['outcome'] == 100 or (instance_stats['outcome'] == -1 and 'total_time\n' in instance_stats):
                solved_times.append(instance_stats['total_time\n'])
                solved_times_size_points.append([instance_stats['total_time\n'],
                                                 instance_stats['p1fluents'] + instance_stats['p1operators'] +
                                                 instance_stats['p2fluents'] + instance_stats['p2operators']])
    ...
    print(f"SIZE : {len(unsat_times_points)} : {len(solved_times_points)}")
...
def main(argv):
    ...
    graphs_data = [(solved_times_points, solved_times_points_nocp, 'solved_by_time',
                    'Number of Isomorphisms found within the Time Limit', 'Time cutoff (s)',
                    'Total # of instances solved'),
                   (unsat_times_points, unsat_times_points_nocp, 'unsat_by_time',
                    'Number of Negative Instances solved within the Time Limit', 'Time cutoff (s)',
                    'Total # of instances solved'),
                   (solved_times_size_points, solved_times_size_points_nocp, 'solved_maxsize_by_time',
                    'Size of the Biggest Positive Instance solved within the Time Limit', 'Time cutoff (s)',
                    '|P| + |P\'|'),
                   (unsat_times_size_points, unsat_times_size_points_nocp, 'unsat_maxsize_by_time',
                    'Size of the Biggest Negative Instance solved within the Time Limit', 'Time cutoff (s)',
                    '|P| + |P\'|')]

    for data, data_nocp, file_name, title, xlabel, ylabel in graphs_data:
        # plt.tight_layout()
        ax = sns.lineplot(x=list(map(lambda x: x[0], data)), y=list(map(lambda x: x[1], data)), linewidth=LINE_WIDTH)  # marker='o'
        ax = sns.lineplot(x=list(map(lambda x: x[0], data_nocp)), y=list(map(lambda x: x[1], data_nocp)), linewidth=LINE_WIDTH)
        ax.set_xlim(xmin=0, xmax=XMAX - 1)
        ax.set_ylim(ymin=0, ymax=YMAX)
        ax.set_xticks(range(0, XMAX, TICKS), labels=[x for x in range(0, XMAX, TICKS)])
    ...
    for domain in [d for d in os.listdir('.') if not os.path.isfile(d)]:
        ...
        for instance_name in [f for f in os.listdir(f"./{domain}/Stats")]:
            ...
            if 'total_time\n' in names:
                assoc = [('Compilation', 'sat_translation'),
                         ('CP', 'sat_constraint_propagation_time'),
                         ('Solving', 'sat_solving')]

                if stats['outcome'] == 10:  # Case where an empty domain is found
                    domain_sums['CP'] += stats['total_time\n']
                else:
                    for key_name, csv_name in assoc:
                        if csv_name not in names:
                            continue
                        domain_sums[key_name] += stats[csv_name]

                if stats['clauses'] + stats['simplified_clauses'] > 0:
                    domain_sums['SimpClausesPerc'] += stats['simplified_clauses'] / (
                            stats['clauses'] + stats['simplified_clauses']) * 100  # Might be /0
        ...
        if instance_stats_mxmin is not None:
            maxmin_stats[domain] = instance_stats_mxmin
        if domain_sums['Count'] > 0:
            averages[domain] = {'CP': domain_sums['CP'] / domain_sums['Count'],
                                'Compilation': domain_sums['Compilation'] / domain_sums['Count'],
                                'Solving': domain_sums['Solving'] / domain_sums['Count'],
                                'Total': domain_sums['Total'] / domain_sums['Count'],
                                'SimpClausesPerc': domain_sums['SimpClausesPerc'] / domain_sums['Count']}
    ...
    with open('biggest.tex', 'w') as biggest_file, \
        ...
        for domain, stats in sorted(list(biggest_stats.items())):
            ...
            biggest_file.write(
                f"\t{domain} & {min(sm1, sm2)} & {max(sm1, sm2)} & {sm} & {min(smm1, smm2)} & {max(smm1, smm2)} & {smm1 + smm2} \\\\\n")

            if not domain.endswith('-nocp'):
                count_file.write(f"\t{domain} & {terminated_stats[domain]['CP']} & {terminated_stats[domain]['NOCP']} ")
                count_file.write(
                    f"& {terminated_stats[domain]['CP'] / terminated_stats[domain]['Total'] * 100:0.1f}\\% ")
                count_file.write(
                    f"& {terminated_stats[domain]['NOCP'] / terminated_stats[domain]['Total'] * 100:0.1f}\\% ")
                count_file.write(f"& {averages[domain]['SimpClausesPerc']:0.1f}\\% \\\\ \n")

                average_file.write(
                    f"\t{domain} & {averages[domain]['CP']:0.1f} & {averages[domain]['Compilation']:0.1f} & {averages[domain]['Solving']:0.1f} & {averages[domain]['Total']:0.1f} \\\\ \n")
