...
"
  and argspecs = [ (* This list enumerates the different flags (-x,-f...)*)
    ...
    ("-po", Arg.Unit (fun _ -> parse_only := 1), ("parse only"));

    ("-nocli", Arg.Unit (fun _ -> cli := 1), ("do not output in command line"));
  ...
  match !parse_only with
    | 0 -> (new Touistparse.prs !problem !domain !options 100 !inputencoding !verbose !cli)#search
    | _ -> 
  ...
  | _ ->
    ...
    | "parser" -> (new Touistplan.t !problem !domain !options 120 !inputencoding solver_code !nodewidth !incrmode !incmin !timeout !verbose)#search
