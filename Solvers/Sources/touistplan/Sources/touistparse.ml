...
class prs (problem:string) (domain:string) (options:string) (encoding : int) (inputencoding : string) (verbose : int) (direct_write : int) =
object (self)
...
(* Génération des ensembles à partir du problème de planification *)
 ...
 Utils.print "Starting to generate problem data...\n";
 let setswrite s =
   ignore (Unix.write setsfile (Bytes.of_string s) 0 (String.length s)) in  
    ...
    match direct_write with
    | 1 ->  setswrite (Printf.sprintf "\n$I = [%s]" (string_of_fluent_array self#init_state));
    ...
    ;
    Utils.print "Done.\n";
    Utils.print "Saved in solvedata/in.sets.txt\n";
    Utils.print "Generation successful\n";
 Unix.close setsfile;
